// var numbers = [2002,2001,2012,2022,2030,2021,2021,2010,2010];
...
//         let temp = numbers[i];
//         // penukan angka dimulai (index 0 ke index 1)
//         numbers[i] = numbers[j]

//         //  penukaran angka (index 1 ke index 0)
//         numbers[j] = temp
//       }
...
function sort(cars) {
const array =[];
  for(let i=0; i<multiCars.length; i++){
    for(let j=i+1; j<multiCars.length; j++){
      if(multiCars[i].year>multiCars[j].year){
        let save = multiCars[i].year
        multiCars[i].year = multiCars[j].year
        multiCars[j].year = save;
        
      }
      
    }
    array.push(multiCars[i])
  }
  return array;
}
console.log(sort(2));
